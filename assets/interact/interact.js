    ...
    if (e.target.parentElement.classList.contains("input-field")) {
        ...
        if(e.target.parentElement.dataset.validatormode == "offline"){
            switch (e.target.parentElement.dataset.type.toLowerCase()) {
                ...
                case "date":
                        window.addEventListener("click", (s) => {
                            if (inputField.value.length < 1 && !inputField.parentElement.contains(s.target) && inputField.parentElement.dataset.type == "date") {
                                e.target.parentElement.classList.remove("valid");
                                e.target.parentElement.classList.remove("alert");
                            }
                        })
...
const interAlertConfirm = ({
    ...
    const renderUI = () => {
        ...
        window.insertBefore(alertUI, body.firstChild);
        return alertUI;
    }
...
const changeTheme = () => {
    const button = document.querySelectorAll(".btn-theme-change");
    button.forEach(btn => {
        btn.addEventListener("click", e => {
            if(checkTheme()){
                body.classList.remove("dark")
                button.forEach( e => {
                    e.children[0].classList.replace("sun", "crescent-moon");
                });
            } else {
                button.forEach( e => {
                    e.children[0].classList.replace("crescent-moon", "sun");
                });
